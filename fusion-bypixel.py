...
# *** Function that fuse tho image with hierarchical fusion method
def hierarchical_fusion(img1, img2):
    ...
    for i in xrange(5, 0, -1):
        ...
        # article
        L = (GE / (gpImgVl[i-1])) + 1
        G = np.dot(np.int16(L), gpImgVl[i - 1])
        lpImgVl.append(G)
    ...
    for i in xrange(5, 0, -1):
        ...
        # L = cv2.subtract(gpImgNir[i - 1], GE)
        L = (GE/ (gpImgNir[i - 1])) + 1
        G = np.dot(np.int32(L), gpImgNir[i - 1])
        lpImgNir.append(G)
        cv2.imshow('Contrast pyramid:', lpImgNir[5-i])
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    # Fusion algorithm
    LS = []
...
print '2 for hierarchical based fusion'
a = input('Choose a number:')
if a == 1:
    laplacian_pyramid_fusion(imgVl, imgNir)
else:
    hierarchical_fusion(imgVl, imgNir)
